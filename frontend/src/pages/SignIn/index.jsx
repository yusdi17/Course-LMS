...
export default function SignInPage() {
    ...
    const {isPending, mutateAsync} = useMutation({
        mutationFn: (payload) => postSignIn(payload)
    })
    ...
    return (
        <div className="relative flex flex-col flex-1 p-[10px]">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-[400px] h-fit rounded-[20px] border border-[#262A56] p-[30px] gap-[30px] bg-[#080A2A] m-auto" >
                ...
                {errors.email?.message && <span className="text-red-500 text-sm -mt-5 ml-3">Harap isi email</span>}
                <div>
                    <div className="flex items-center gap-3 w-full rounded-full border p-[14px_20px] transition-all duration-300 focus-within:border-[#8661EE] focus-within:shadow-[-10px_-6px_10px_0_#7F33FF_inset] bg-[#070B24] border-[#24283E] shadow-[-10px_-6px_10px_0_#181A35_inset]">
                    ...
                    {errors.password?.message && <span className="text-red-500 text-sm -mt-5 ml-3">Harap isi password</span>}
                    <div className="flex justify-end mt-[10px]">
                        <Link to="#" className="text-sm leading-[21px] text-[#662FFF] hover:underline">Forgot Password</Link>
                    </div>
                </div>
                <hr className="border-[#262A56]" />
                <button type="submit" disabled={isPending} className="w-full rounded-full border p-[14px_20px] text-center font-semibold text-white bg-[#662FFF] border-[#8661EE] shadow-[-10px_-6px_10px_0_#7F33FF_inset]">
                    Sign In
                </button>
            </form>
        </div>
